...
wx-still 是 KnowWind 的微信数据源插件。它是一个后台服务，没有独立的用户界面，必须配合 KnowWind 核心平台使用。

**依赖关系**：必须先安装 KnowWind，再安装 wx-still。
...
macOS（launchd）：
```xml
<!-- ~/Library/LaunchAgents/com.knowwind.wx-still.plist -->
<key>ProgramArguments</key>
<array>
    <string>python3</string>
    <string>/path/to/wx-still/server/main.py</string>
</array>
<key>RunAtLoad</key>
<true/>
```
...
Windows（任务计划程序）：
```
schtasks /create /tn "wx-still" /tr "python3 /path/to/wx-still/server/main.py" /sc onlogon
```

Linux（systemd）：
```ini
# ~/.config/systemd/user/wx-still.service
[Service]
ExecStart=python3 /path/to/wx-still/server/main.py
Restart=on-failure
