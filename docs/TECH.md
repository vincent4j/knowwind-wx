...
│   ├── insights.py            # 规则过滤 + LLM 精提取 + 推送
│   └── db.py                  # SQLite 操作
├── ui/
│   └── index.html             # 配置界面（Vue 3 CDN，注册进 KnowWind）
└── scripts/
    ├── extract-insights.py    # 独立 CLI 工具：stdin 消息 → stdout insights
    └── push-insights.py       # 独立 CLI 工具：stdin insights → 推送 KnowWind
```
...
### 第一步：拉取消息（`server/main.py: _run_collect`）

```python
# 从数据库读取 last_fetched_at
# 调用 wechat-decrypt API（按群逐一拉取）
GET {WECHAT_DECRYPT_URL}/api/history?chat={group_name}&since={last_fetched_at}&limit=2000

# 采集完成后更新 last_fetched_at，写入 fetch_logs
```
...
### 第二步：规则粗过滤（`server/insights.py: filter_candidates`）
...
### 第三步：LLM 精提取（`server/insights.py: extract_insights`）
...
### 第四步：推送（`server/insights.py: push_insights`）
