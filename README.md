...
```
帮我安装 knowwind-wx 微信插件：clone https://github.com/vincent4j/knowwind-wx，安装依赖，注册开机自启。
```
...
knowwind-wx 通过 [wechat-decrypt](https://github.com/ylytdeng/wechat-decrypt) 读取本机微信消息。
...
knowwind-wx 是独立进程，需要手动启动（或配置开机自启）。

```bash
# 1. 确认 wechat-decrypt 在运行
curl -s http://localhost:5678
# 有响应即可（404 也行，说明端口在监听）

# 2. 启动 knowwind-wx（注意：必须用 python3.10）
cd /path/to/knowwind-wx
python3.10 -m uvicorn server.main:app --host 127.0.0.1 --port 8001
...
> **为什么用 `python3.10` 而不是 `python3`？**
> 本机 `python3` 指向 Homebrew 的 3.14（PEP 668 限制，无法 pip install），
> 依赖装在 3.10 下。直接用 `python3` 会报 `No module named uvicorn`。

启动后 KnowWind 心跳自动检测，在数据源管理页显示「在线」。
